...
public class MusicaController {
	...
	@RequestMapping(value = "/musica", method = RequestMethod.HEAD)
	public ResponseEntity<Void> headById(@PathVariable String id) {
		
		Optional<Musica> musicaOptional = musicaService.findById(id);
		if (musicaOptional.isPresent()) {
			System.out.println("El recurso con ID " + id + " existe en la base de datos.");
			HttpHeaders headers = new HttpHeaders();
			headers.add("Content-type", "Application/Json");
			return new ResponseEntity<>(headers,HttpStatus.OK);
		} else {
			System.out.println("El recurso con ID " + id + " no existe en la base de datos.");
			return ResponseEntity.notFound().build();
		}
